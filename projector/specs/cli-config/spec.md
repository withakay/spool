...
## Purpose
Provide a user-friendly CLI interface for viewing and modifying global Projector configuration settings without manually editing JSON files.
## Requirements
...
- **WHEN** user executes `projector config --help`
...
- **WHEN** user executes `projector config path`
...
- **WHEN** user executes `projector config get <key>` with a valid top-level key
- **THEN** print the raw value only (no labels or formatting)
- **AND** exit with code 0

#### Scenario: Get nested key with dot notation

- **WHEN** user executes `projector config get featureFlags.someFlag`
- **THEN** traverse the nested structure using dot notation
- **AND** print the value at that path

#### Scenario: Get non-existent key

- **WHEN** user executes `projector config get <key>` with a key that does not exist
- **THEN** print nothing (empty output)
- **AND** exit with code 1

#### Scenario: Get object value

- **WHEN** user executes `projector config get <key>` where the value is an object
...
- **WHEN** user executes `projector config set <key> <value>`
...
- **WHEN** user executes `projector config set <key> true` or `projector config set <key> false`
...
- **WHEN** user executes `projector config set <key> <value> --string`
...
- **WHEN** user executes `projector config set featureFlags.newFlag true`
...
- **WHEN** user executes `projector config unset <key>`
...
- **WHEN** user executes `projector config reset --all`
...
- **WHEN** user executes `projector config reset --all -y`
- **THEN** reset without prompting for confirmation

#### Scenario: Reset without --all flag

- **WHEN** user executes `projector config reset` without `--all`
...
- **WHEN** user executes `projector config edit`
...
- **WHEN** user executes `projector config set someFutureKey 123`
...
- **WHEN** user executes `projector config set someFutureKey 123 --allow-unknown`
- **THEN** the value is saved successfully
- **AND** exit with code 0

#### Scenario: Invalid feature flag value rejected

- **WHEN** user executes `projector config set featureFlags.someFlag notABoolean`
...
- **WHEN** user executes `projector config --scope project <subcommand>`
