...
- **WHEN** user runs `projector status --change <id>`
...
- **WHEN** user runs `projector status --change <id> --json`
...
- **WHEN** user runs `projector status --change <id>` on a change with no artifacts
...
- **WHEN** user runs `projector status` without `--change`
...
- **WHEN** user runs `projector status --change unknown-id`
- **AND** directory `projector/changes/unknown-id/` does not exist
...
- **WHEN** user runs `projector instructions <artifact> --change <id>`
...
- **WHEN** user runs `projector instructions <artifact> --change <id> --json`
- **THEN** the system outputs JSON matching ArtifactInstructions interface

#### Scenario: Unknown artifact

- **WHEN** user runs `projector instructions unknown-artifact --change <id>`
...
- **WHEN** user runs `projector instructions proposal --change <id>` on a scaffolded change
...
- **WHEN** user runs `projector templates`
...
- **WHEN** user runs `projector new change add-feature`
- **THEN** the system creates `projector/changes/add-feature/` directory

#### Scenario: Invalid change name
- **WHEN** user runs `projector new change "Add Feature"` with invalid name
- **THEN** the system displays validation error with guidance

#### Scenario: Duplicate change name
- **WHEN** user runs `projector new change existing-change` for an existing change
- **THEN** the system displays an error indicating the change already exists

#### Scenario: Create with description
- **WHEN** user runs `projector new change add-feature --description "Add new feature"`
...
- **WHEN** user runs `projector status --change <id> --schema tdd`
...
The system SHALL generate schema-aware apply instructions via `projector instructions apply`.

#### Scenario: Generate apply instructions

- **WHEN** user runs `projector instructions apply --change <id>`
...
- **WHEN** user runs `projector instructions apply --change <id> --json`
- **THEN** the system outputs JSON with:
...
**Reason**: Redundant with Status Command - `projector status` already shows which artifacts are ready (status: "ready") vs blocked vs done.

**Migration**: Use `projector status --change <id> --json` and filter artifacts with `status: "ready"` to find artifacts that can be created next.
