...
- **WHEN** executing `projector change show` without arguments
...
- **WHEN** executing `projector change show` without a change name
...
- **WHEN** executing `projector change validate` without arguments
- **THEN** display an interactive list of available changes
...
- **GIVEN** stdin is not a TTY or `--no-interactive` is provided or environment variable `PROJECTOR_INTERACTIVE=0`
- **WHEN** executing `projector change validate` without a change name
- **THEN** do not prompt interactively
- **AND** print the existing hint including available change IDs
- **AND** set `process.exitCode = 1`
