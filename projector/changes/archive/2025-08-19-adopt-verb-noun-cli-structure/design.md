...
2. Add `--specs`/`--changes` to `projector list`, with `--changes` as default.
3. Show deprecation warnings for `projector change list` and, more generally, for any `projector change ...` and `projector spec ...` subcommands.
...
## Out of Scope
JSON output parity for `projector list` across modes and `show --specs/--changes` discovery are follow-ups.
