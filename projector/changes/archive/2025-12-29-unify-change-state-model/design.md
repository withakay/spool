...
|--------|---------|---------|
| **Task Progress** | Track implementation work | `projector view`, `projector list` |
| **Artifact Progress** | Track planning/spec work | `projector status`, `projector next` |
...
async function validateChangeExists(changeName: string, projectRoot: string): Promise<string> {
  const changePath = path.join(projectRoot, 'projector', 'changes', changeName);
  ...
  if (!fs.existsSync(changePath) || !fs.statSync(changePath).isDirectory()) {
    ...
    const entries = await fs.promises.readdir(
      path.join(projectRoot, 'projector', 'changes'),
      { withFileTypes: true }
    );
...
```bash
# Before
$ projector new change foo
$ projector status --change foo
