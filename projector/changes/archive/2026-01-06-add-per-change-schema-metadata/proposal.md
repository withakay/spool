...
**Scope: Experimental artifact workflow only** (`projector new change`, `projector status`, `projector instructions`, `projector templates`)

- Store schema choice in `.projector.yaml` metadata file when creating a change via `projector new change`
...
- Add `--schema` option to `projector new change` command

**Not affected**: Legacy commands (`projector validate`, `projector archive`, `projector list`, `projector show`)
...
- **Backward compatible**: Changes without `.projector.yaml` fall back to `spec-driven` default
