...
The system SHALL store and validate per-change metadata in `.projector.yaml` files using a Zod schema.

#### Scenario: Metadata file created with new change

- **WHEN** user runs `projector new change add-feature --schema tdd`
- **THEN** the system creates `.projector.yaml` in the change directory
- **AND** the file contains `schema: tdd` and `created: <YYYY-MM-DD>`

#### Scenario: Metadata validated on read

- **WHEN** the system reads `.projector.yaml`
...
- **WHEN** `.projector.yaml` contains invalid YAML or missing required fields
- **THEN** the system displays a Zod validation error with details

#### Scenario: Missing metadata file

- **WHEN** a change directory has no `.projector.yaml` file
...
- **AND** creates `.projector.yaml` with `schema: spec-driven` (default)

#### Scenario: Create change with schema

- **WHEN** user runs `projector new change add-feature --schema tdd`
- **THEN** the system creates `projector/changes/add-feature/` directory
- **AND** creates `.projector.yaml` with `schema: tdd`

#### Scenario: Invalid schema on create

- **WHEN** user runs `projector new change add-feature --schema unknown`
...
- **WHEN** user runs `projector status --change <id>` without `--schema`
- **AND** the change has `.projector.yaml` with `schema: tdd`
- **THEN** the system uses the `tdd` schema

#### Scenario: Explicit schema overrides metadata

- **WHEN** user runs `projector status --change <id> --schema spec-driven`
- **AND** the change has `.projector.yaml` with `schema: tdd`
...
- **AND** the change has no `.projector.yaml` file
