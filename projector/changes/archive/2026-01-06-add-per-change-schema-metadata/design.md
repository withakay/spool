...
The metadata file (`.projector.yaml`) will be validated with this Zod schema:
...
/**
 * Schema for per-change metadata stored in .projector.yaml
 */
...
**Location:** `projector/changes/<name>/.projector.yaml`
...
- `projector.json` - YAML matches existing schema files
...
2. **`.projector.yaml` metadata** (persisted choice)
...
No migration needed:
- Existing changes without `.projector.yaml` continue to work (use default)
- New changes created with `projector new change --schema X` get metadata file

## Open Questions

- Should `projector new change` prompt for schema interactively if not specified? (Leaning no - default is fine)
