...
The missing piece is archiving. The existing `projector archive` command works but:
...
- Preserve `.projector.yaml` schema metadata in archive

**Non-Goals:**
- Replacing the existing `projector archive` CLI command
...
**Rationale**: The existing `projector archive` CLI command already handles the core archive functionality (moving to archive folder, date prefixing). The OPSX version just needs different pre-archive checks and optional sync prompting, which are agent behaviors better suited to a skill.

**Alternatives considered**:
- Adding flags to `projector archive` (e.g., `--experimental`) - Rejected: adds complexity to CLI, harder to maintain two code paths
- New CLI command `projector archive-experimental` - Rejected: unnecessary duplication, agent skills are the OPSX pattern
...
The skill will use `projector status --change "<name>" --json` to check artifact completion instead of just validating proposal.md and specs.
...
Same archive behavior as regular command: move to `projector/changes/archive/YYYY-MM-DD-<name>/`.

**Rationale**: Consistency with existing archive convention. The `.projector.yaml` file moves with the change, preserving schema metadata.

## Risks / Trade-offs

**Risk**: Users confused about when to use `/opsx:archive` vs `projector archive`
→ **Mitigation**: Documentation should clarify: use `/opsx:archive` if you've been using the OPSX workflow, use `projector archive` otherwise. Both produce the same archived result.
...
→ **Accepted**: This is intentional. OPSX philosophy is agent-driven merging. If user wants programmatic application, use `projector archive` instead.
