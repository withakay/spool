# Update Projector Agent Instructions

## Why

The current Projector agent instructions need updates to follow best practices for AI assistant instructions (brevity, clarity, removing ambiguity), ensure CLI commands are current with the actual implementation, and properly document the three-stage workflow pattern that agents should follow.
...
- **Complete command documentation** with all current functionality:
  - `projector init [path]` - Initialize Projector in a project
  - `projector list` - List all active changes (default)
  - `projector list --specs` - List all specifications
  - `projector show [item]` - Display change or spec with auto-detection
  - `projector show` - Interactive mode for selection
  - `projector diff [change]` - Show spec differences for a change
  - `projector validate [item]` - Validate changes or specs
  - `projector archive [change]` - Archive completed change after deployment
  - `projector update [path]` - Update Projector instruction files
...
- **Remove deprecated command references** (noun-first patterns like `projector change show`)
- **Add concrete examples** for each command variation
- **Document debugging commands**:
  - `projector show [change] --json --deltas-only` for inspecting deltas
  - `projector validate [change] --strict` for comprehensive validation
...
- **Delta detection debugging**:
  - Use `projector show [change] --json --deltas-only` to inspect parsed deltas
...
- **Spec discovery workflow** - Always check existing specs before creating new ones:
  - Use `projector list --specs` to see all current specs
...
- Affected documentation:
  - `projector/README.md` - Main Projector instructions
  - CLAUDE.md files generated by `projector init` command

Note: This is a tooling/infrastructure change that doesn't require spec updates. When archiving, use `projector archive update-agent-instructions --skip-specs`.