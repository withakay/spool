...
state via `process.cwd()` and `projector/changes`. This makes outcomes depend on
...
- Each suite that touches filesystem discovery creates a temporary directory:
  - `projector/changes/sample-change/proposal.md`
  - `projector/changes/sample-change/specs/sample/spec.md`
...
- Tests that previously depended on repo state now:
  ...
  - Do not read real `projector/changes` during execution.
