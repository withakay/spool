...
- **WHEN** executing `projector show` without arguments
...
- **WHEN** executing `projector show` without arguments
- **THEN** do not prompt
- **AND** print a helpful hint with examples for `projector show <item>` or `projector change/spec show`
- **AND** exit with code 1

#### Scenario: Direct item display

- **WHEN** executing `projector show <item-name>`
...
- **AND** if it matches both, print an ambiguity error and suggest `--type change|spec` or using `projector change show`/`projector spec show`
- **AND** if it matches neither, print not-found with nearest-match suggestions

#### Scenario: Explicit type override

- **WHEN** executing `projector show --type change <item>`
- **THEN** treat `<item>` as a change ID and show it (skipping auto-detection)

- **WHEN** executing `projector show --type spec <item>`
...
- **WHEN** executing `projector show <item> --json`
...
- **WHEN** showing a change with `projector show <change-name> --deltas-only`
...
- **WHEN** showing a spec with `projector show <spec-id> --requirements`
