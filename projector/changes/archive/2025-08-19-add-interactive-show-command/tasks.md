...
- Respect `--no-interactive` and `PROJECTOR_INTERACTIVE=0` consistently.
...
Acceptance:
- `projector show` exists and prints a helpful hint in non-interactive contexts when no args.
...
Acceptance:
- `projector show <change-id> --json --deltas-only` matches `projector change show <id> --json --deltas-only` output.
- `projector show <spec-id> --json --requirements` matches `projector spec show <id> --json --requirements` output.
...
- [x] `src/commands/change.ts` → extend `show(changeName?: string, options?: { json?: boolean; requirementsOnly?: boolean; deltasOnly?: boolean; noInteractive?: boolean })`:
  ...
  - Non-interactive fallback: keep current behavior (print available IDs + `projector change list` hint, set `process.exitCode = 1`).
...
Acceptance:
- `projector change show` in non-interactive prints list hint and exits non-zero.
- `projector spec show` in non-interactive prints missing-arg error and exits non-zero.
...
- [x] Top-level `show` hint (non-interactive no-arg):
  - Lines include: `projector show <item>`, `projector change show`, `projector spec show`, and "Or run in an interactive terminal.".
...
## 7) Tests
Add tests mirroring existing patterns (non-TTY simulation via `PROJECTOR_INTERACTIVE=0`).
...
- [x] `test/commands/change.interactive-show.test.ts` (non-interactive fallback)
  - Ensure `projector change show` without args prints available IDs + list hint and non-zero exit.
- [x] `test/commands/spec.interactive-show.test.ts` (non-interactive fallback)
  - Ensure `projector spec show` without args prints missing-arg error and non-zero exit.
...
- [x] Update `projector/README.md` usage examples to include the new `show` command with type detection and flags.
