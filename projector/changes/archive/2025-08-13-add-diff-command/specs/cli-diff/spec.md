...
The `projector diff` command provides developers with a visual comparison between proposed spec changes and the current deployed specs.

## Command Syntax

```bash
projector diff [change-name]
```
...
WHEN changes directory doesn't exist THEN display "No Projector changes directory found"

## Examples

```bash
# View diff for specific change
$ projector diff add-auth-feature
