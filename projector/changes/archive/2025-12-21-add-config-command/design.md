...
The `global-config` spec defines how Projector reads/writes `config.json`, but users currently must edit it by hand. This command provides a CLI interface to that config.
...
**Examples:**
```bash
projector config get featureFlags              # Returns object
projector config get featureFlags.experimental # Returns nested value
projector config set featureFlags.newFlag true
```
...
**Rationale:**
- Raw output enables piping: `VAR=$(projector config get key)`
