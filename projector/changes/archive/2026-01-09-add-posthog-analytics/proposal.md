...
Projector currently has no visibility into how the tool is being used. Without analytics, we cannot:
...
- Store anonymous ID in global config (`~/.config/projector/config.json`)
- Respect `DO_NOT_TRACK` and `PROJECTOR_TELEMETRY=0` environment variables
