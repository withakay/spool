...
## Summary
- Enable Projector to generate and update custom slash commands for supported coding agents (Claude Code and Cursor).
- Provide three slash commands aligned with Projector's workflow: proposal (start a change proposal), apply (implement), and archive.
- Share slash command templating between agents to make future extensions simple.

## Motivation
Developers use different coding agents and editors. Having consistent slash commands across tools for the Projector workflow reduces friction and ensures a standard way to trigger the workflow. Supporting both Claude Code and Cursor now lays a foundation for future agents that introduce slash command features.

## Proposal
1. During `projector init`, when a user selects a supported tool, generate slash command configuration for three Projector workflow stages:
   - Claude (namespaced): `/projector/proposal`, `/projector/apply`, `/projector/archive`.
   - Cursor (flat, prefixed): `/projector-proposal`, `/projector-apply`, `/projector-archive`.
   - Semantics:
   ...
   - Each command file MUST embed concise, step-by-step instructions sourced from `projector/README.md` (see Template Content section).
2. Store slash command files per tool:
   - Claude Code: `.claude/commands/projector/{proposal,apply,archive}.md`
   - Cursor: `.cursor/commands/{projector-proposal,projector-apply,projector-archive}.md`
...
3. Command file format and metadata:
   ...
   - Place Projector markers around the body only, never inside frontmatter.
   - Keep the visible slash name, file name, and any frontmatter `name`/`id` consistently aligned (e.g., `proposal`, `projector-proposal`).
   - Namespacing: categorize these under “Projector” and prefer unique IDs (e.g., `projector-proposal`) to avoid collisions.
...
5. During `projector update`, refresh only existing slash command files (per-file basis) within markers; do not create missing files or new tools.
...
- Introduce `SlashCommandConfigurator` to manage multiple files per tool.
  ...
  - Provide `generateAll(projectPath, projectorDir)` for init and `updateExisting(projectPath, projectorDir)` for update.
- Per-tool adapters add only frontmatter and pathing; bodies come from shared templates.
- Templates live in `TemplateManager` with helpers that extract concise, authoritative snippets from `projector/README.md`.
...
- Markers MUST wrap only the Markdown body contents:
  ...
  - Then `<!-- PROJECTOR:START -->` … body … `<!-- PROJECTOR:END -->`.
...
- Directory creation for `.claude/commands/projector/` and `.cursor/commands/` is the configurator’s responsibility.

### Command Naming & UX
- Claude Code: use namespacing in the slash itself for readability and grouping: `/projector/proposal`, `/projector/apply`, `/projector/archive`.
- Cursor: use flat names with an `projector-` prefix: `/projector-proposal`, `/projector-apply`, `/projector-archive`. Group via `category: Projector` when supported.
- Consistency: align file names, visible slash names, and any frontmatter `id` (e.g., `id: projector-apply`).
...
- Allow users to customize command names and categories during `projector init`.
...
### Claude Code: `.claude/commands/projector/proposal.md`
```markdown
---
...
Slash invocation: `/projector/proposal` (namespaced)

### Cursor: `.cursor/commands/projector-proposal.md`
```markdown
---
name: /projector-proposal
...
Slash invocation: `/projector-proposal` (flat, prefixed)

## Template Content
Templates should be brief, actionable, and sourced from `projector/README.md` to avoid duplication. Each command body includes:
...
- Pointers to `projector show`, `projector list`, and troubleshooting tips when validation fails.
