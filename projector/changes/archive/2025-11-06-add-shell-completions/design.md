...
```bash
# After installing: projector completion install bash
projector val<TAB>           # Completes to "projector validate"
projector validate <TAB><TAB>  # Shows: --all --changes --specs --strict --json [change-ids] [spec-ids]
projector show add-<TAB><TAB>  # Shows all changes starting with "add-"
```
...
**Projector Integration:**
```zsh
# After installing: projector completion install zsh
projector val<TAB>    # Shows menu with "validate" and "view" highlighted
projector show <TAB>  # Shows menu with all change IDs and spec IDs, categorized
```
...
**Projector Integration:**
```fish
# After installing: projector completion install fish
projector val       # Gray suggestion shows "validate" immediately
projector show a    # Real-time suggestions for changes starting with "a"
projector <TAB>     # Shows all commands with descriptions in paged menu
```
...
**Projector Integration:**
```powershell
# After installing: projector completion install powershell
projector val<TAB>       # Cycles: validate → view → validate
projector show <TAB>     # Cycles through change IDs one by one
projector <Ctrl+Space>   # Shows IntelliSense menu with all commands
```
...
class CompletionProvider {
  ...
  isProjectorProject(): boolean {
    // Check for projector/ directory
  }
}
```
...
- Graceful degradation when outside Projector projects
...
│       ├── command-registry.ts    # Single source of truth for Projector commands
...
if (isOhMyZshInstalled()) {
  // Install to ~/.oh-my-zsh/custom/completions/_projector
  // Automatically loaded by Oh My Zsh
} else {
  // Install to ~/.zsh/completions/_projector
...
2. **Not in Projector project** - Skip dynamic completions, only offer static commands
...
3. **Manual Testing**
   ...
   - Tab completion behavior in Projector projects
