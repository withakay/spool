...
- [x] Create `src/core/completions/command-registry.ts` with `COMMAND_REGISTRY` constant defining all Projector commands, flags, and metadata
...
- [x] Implement installation to `~/.oh-my-zsh/custom/completions/_projector` for Oh My Zsh
- [x] Implement fallback installation to `~/.zsh/completions/_projector` with `fpath` updates
...
- [x] Implement dynamic completion helper functions in Zsh generator (`_projector_complete_changes`, `_projector_complete_specs`, `_projector_complete_items`)
...
- [ ] Test dynamic change/spec ID completion in real Projector projects
- [ ] Verify completion cache behavior (2-second TTL)
- [ ] Test behavior outside Projector projects (should skip dynamic completions)
- [x] Update `projector --help` output to include completion command (automatically done via Commander)
