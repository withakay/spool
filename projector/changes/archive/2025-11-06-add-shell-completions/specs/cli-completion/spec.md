...
- **WHEN** user executes `projector completion zsh`
...
- **THEN** remove `~/.oh-my-zsh/custom/completions/_projector` if Oh My Zsh is detected
...
- **WHEN** `projector completion install` cannot detect current shell or detects non-Zsh shell
- **THEN** display error: "Could not detect Zsh. Please specify explicitly: projector completion install zsh"
- **AND** exit with code 1
...
- **AND** allow redirection to files: `projector completion zsh > /path/to/_projector`
