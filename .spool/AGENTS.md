...
1. Choose a unique verb-led `change-id` and scaffold `proposal.md`, `tasks.md`, optional `design.md`, and spec deltas under `.spool/changes/<id>/`.
1. Draft spec deltas using `## ADDED|MODIFIED|REMOVED Requirements` with at least one `#### Scenario:` per requirement.
1. Run `spool validate <id> --strict` and resolve any issues before sharing the proposal.
...
- Coverage target: 80% (guidance; override in `.spool/config.json` via `defaults.testing.coverage.target_percent`).
...
1. **Update statuses** - Prefer `spool tasks start|complete` for enhanced tasks.md; for legacy checkbox lists, set `- [x]` so the list reflects reality
