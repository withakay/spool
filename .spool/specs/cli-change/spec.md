...
- **WHEN** executing `spool change show update-error --json`
...
- **WHEN** executing `spool change list`
...
- **WHEN** executing `spool change show update-error --requirements-only`
...
- **WHEN** executing `spool change validate update-error`
...
- **AND** show deprecation notice: "Note: 'spool list' is deprecated. Use 'spool change list' instead."

#### Scenario: Legacy list with --all flag

- **WHEN** executing `spool list --all`
...
- **WHEN** executing `spool change show` without arguments
...
- **WHEN** executing `spool change show` without a change name
...
- **WHEN** executing `spool change validate` without arguments
- **THEN** display an interactive list of available changes
...
- **GIVEN** stdin is not a TTY or `--no-interactive` is provided or environment variable `SPOOL_INTERACTIVE=0`
- **WHEN** executing `spool change validate` without a change name
- **THEN** do not prompt interactively
- **AND** print the existing hint including available change IDs
- **AND** set `process.exitCode = 1`
