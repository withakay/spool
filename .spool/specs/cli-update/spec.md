...
As a developer using Spool, I want to update the Spool instructions in my project when new versions are released, so that I can benefit from improvements to AI agent instructions.
## Requirements
...
- **AND** if a root-level stub (`AGENTS.md`/`CLAUDE.md`) exists, refresh it so it points to `@/spool/AGENTS.md`

### Requirement: Prerequisites

The command SHALL require an existing Spool structure before allowing updates.

#### Scenario: Checking prerequisites

- **GIVEN** the command requires an existing `spool` directory (created by `spool init`)
- **WHEN** the `spool` directory does not exist
- **THEN** display error: "No Spool directory found. Run 'spool init' first."
...
- **AND** if a root-level stub exists, update the managed block content so it keeps directing teammates to `@/spool/AGENTS.md`
...
- **AND** update only the Spool-managed sections inside existing AI tool files, leaving user-authored content untouched
...
- **THEN** replace `spool/AGENTS.md` with the latest template
- **AND** if a root-level stub exists, refresh it so it still directs contributors to `@/spool/AGENTS.md`
...
- **AND** preserve any user customizations outside the Spool managed markers
...
- **WHEN** `.continue/prompts/` contains `spool-proposal.prompt`, `spool-apply.prompt`, and `spool-archive.prompt`
- **THEN** refresh each file using shared templates
- **AND** ensure templates include instructions for the relevant workflow stage
...
- **WHEN** a user invokes `/spool:archive <change-id>` with a change ID
- **THEN** the template SHALL instruct the AI to validate the provided change ID against `spool list`
...
- **WHEN** a user invokes `/spool:archive` without providing a change ID
- **THEN** the template SHALL instruct the AI to identify the change ID from context or by running `spool list`
...
- **AND** the default directory name `spool` SHALL be used
