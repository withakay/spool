...
The `spool list` command SHALL provide developers with a quick overview of all active changes in the project, showing their names and task completion status.
...
- **WHEN** `spool list` is executed without flags
- **THEN** scan the `spool/changes/` directory for change directories
- **AND** exclude the `archive/` subdirectory from results
- **AND** parse each change's `tasks.md` file to count task completion

#### Scenario: Scanning for specs

- **WHEN** `spool list --specs` is executed
- **THEN** scan the `spool/specs/` directory for capabilities
...
- **WHEN** `spool/changes/` directory doesn't exist
- **THEN** display error: "No Spool changes directory found. Run 'spool init' first."
...
This command provides that visibility with minimal effort, following Spool's philosophy of simplicity and clarity.
