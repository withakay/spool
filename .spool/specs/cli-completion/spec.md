...
- **WHEN** user executes `spool completion --help`
...
- **THEN** discover active changes from `spool/changes/` directory
- **AND** exclude archived changes in `spool/changes/archive/`
- **AND** return change IDs as completion suggestions
- **AND** only provide suggestions when inside an Spool-enabled project

#### Scenario: Completing spec IDs

- **WHEN** completing arguments for commands that accept spec names (show, validate)
- **THEN** discover specs from `spool/specs/` directory
...
- **WHEN** user requests completions outside an Spool project
...
- **WHEN** user executes `spool completion install zsh`
...
- **AND** write completion script to `~/.oh-my-zsh/custom/completions/_spool`
...
- **WHEN** user executes `spool completion install zsh` and Oh My Zsh is not detected
- **THEN** create completions directory at `~/.zsh/completions/` if it doesn't exist
- **AND** write completion script to `~/.zsh/completions/_spool`
...
- **WHEN** user executes `spool completion install bash`
- **THEN** detect if bash-completion is installed by checking for `/usr/share/bash-completion` or `/etc/bash_completion.d`
- **AND** if bash-completion is available, write to `/etc/bash_completion.d/spool` (with sudo) or `~/.local/share/bash-completion/completions/spool`
- **AND** if bash-completion is not available, write to `~/.bash_completion.d/spool` and source it from `~/.bashrc`
...
- **WHEN** user executes `spool completion install fish`
- **THEN** create Fish completions directory at `~/.config/fish/completions/` if it doesn't exist
- **AND** write completion script to `~/.config/fish/completions/spool.fish`
...
- **WHEN** user executes `spool completion install powershell`
...
- **WHEN** user executes `spool completion install` without specifying a shell
...
- **WHEN** user executes `spool completion uninstall zsh`
...
- **AND** if user confirms, remove `~/.oh-my-zsh/custom/completions/_spool` if Oh My Zsh is detected
- **AND** remove `~/.zsh/completions/_spool` if standard Zsh setup is detected
- **AND** remove fpath modifications from `~/.zshrc` using marker-based removal
- **AND** display success message

#### Scenario: Uninstalling Bash completion

- **WHEN** user executes `spool completion uninstall bash`
...
- **WHEN** user executes `spool completion uninstall fish`
- **THEN** prompt for confirmation (unless `--yes` flag provided)
- **AND** if user confirms, remove `~/.config/fish/completions/spool.fish`
- **AND** display success message (no config file modification needed)

#### Scenario: Uninstalling PowerShell completion

- **WHEN** user executes `spool completion uninstall powershell`
- **THEN** prompt for confirmation (unless `--yes` flag provided)
...
- **WHEN** user executes `spool completion uninstall` without specifying a shell
...
- **AND** implement methods:
  ...
  - `isSpoolProject(): boolean` - Checks if current directory is Spool-enabled
...
- **AND** allow redirection to files: `spool completion generate zsh > /path/to/_spool`
