...
## Purpose
Provide programmatic utilities for creating and validating Spool change directories.
## Requirements
...
- **THEN** the system creates `spool/changes/add-auth/` directory

#### Scenario: Duplicate change rejected
- **WHEN** `createChange(projectRoot, 'add-auth')` is called and `spool/changes/add-auth/` already exists
- **THEN** the system throws an error indicating the change already exists

#### Scenario: Creates parent directories if needed
- **WHEN** `createChange(projectRoot, 'add-auth')` is called and `spool/changes/` does not exist
