...
- **WHEN** executing `spool validate --all`
- **THEN** validate all changes in `.spool/changes/` (excluding archive)
- **AND** validate all specs in `.spool/specs/`
...
- **WHEN** executing `spool validate --specs`
- **THEN** validate all specs in `.spool/specs/`
...
- **WHEN** executing `spool validate --all --strict`
...
- **WHEN** executing `spool validate --all --json`
...
- **WHEN** executing `spool validate --all --json` (or `--changes` / `--specs`)
...
- **WHEN** `spool validate` is executed with `--no-interactive` or with environment `SPOOL_INTERACTIVE=0`
