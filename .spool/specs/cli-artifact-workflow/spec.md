...
- **WHEN** user runs `spool instructions <artifact> --change <id>`
...
- **WHEN** user runs `spool instructions <artifact> --change <id> --json`
- **THEN** the system outputs JSON matching ArtifactInstructions interface

#### Scenario: Unknown artifact

- **WHEN** user runs `spool instructions unknown-artifact --change <id>`
...
- **WHEN** user runs `spool instructions proposal --change <id>` on a scaffolded change
...
The system SHALL generate schema-aware apply instructions via `spool instructions apply`.

#### Scenario: Generate apply instructions

- **WHEN** user runs `spool instructions apply --change <id>`
...
- **WHEN** user runs `spool instructions apply --change <id> --json`
- **THEN** the system outputs JSON with:
