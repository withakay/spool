...
- Respect `--no-interactive` and `SPOOL_INTERACTIVE=0` consistently.
...
Acceptance:
- `spool show` exists and prints a helpful hint in non-interactive contexts when no args.
...
Acceptance:
- `spool show <change-id> --json --deltas-only` matches `spool change show <id> --json --deltas-only` output.
- `spool show <spec-id> --json --requirements` matches `spool spec show <id> --json --requirements` output.
...
- [x] `src/commands/change.ts` → extend `show(changeName?: string, options?: { json?: boolean; requirementsOnly?: boolean; deltasOnly?: boolean; noInteractive?: boolean })`:
  ...
  - Non-interactive fallback: keep current behavior (print available IDs + `spool change list` hint, set `process.exitCode = 1`).
...
Acceptance:
- `spool change show` in non-interactive prints list hint and exits non-zero.
- `spool spec show` in non-interactive prints missing-arg error and exits non-zero.
...
- [x] Top-level `show` hint (non-interactive no-arg):
  - Lines include: `spool show <item>`, `spool change show`, `spool spec show`, and "Or run in an interactive terminal.".
...
## 7) Tests
Add tests mirroring existing patterns (non-TTY simulation via `SPOOL_INTERACTIVE=0`).
...
- [x] `test/commands/change.interactive-show.test.ts` (non-interactive fallback)
  - Ensure `spool change show` without args prints available IDs + list hint and non-zero exit.
- [x] `test/commands/spec.interactive-show.test.ts` (non-interactive fallback)
  - Ensure `spool spec show` without args prints missing-arg error and non-zero exit.
...
- [x] Update `spool/README.md` usage examples to include the new `show` command with type detection and flags.
