...
- Deprecate noun-based top-level commands: `spool spec ...` and `spool change ...`.
...
- Keep `spool spec` and `spool change` available with deprecation warnings for one release cycle.
...
- **Affected specs**:
  ...
  - `spool-conventions`: Add explicit requirement establishing verb–noun CLI design and deprecation guidance
- **Affected code**:
...
- Document new usage in `spool/README.md` and CLI help.
