...
2. Run `bun install` to generate `bun.lock` from `pnpm-lock.yaml`
3. Validate build/test/lint workflows:
   ```bash
   bun install
   bun run build
   bun run test
   bun run lint
   ```
4. Compare build artifacts and test results with pnpm baseline
5. Commit `bun.lock` when validated
...
3. Update `test/helpers/run-cli.ts`:
...
2. Update `.github/workflows/release-prepare.yml`:
   - Same setup-bun swap
...
2. Update `AGENTS.md` Makefile documentation
3. Update any templates or examples in `docs/` and `schemas/`
4. Remove or archive `pnpm-lock.yaml`

### Phase 6: Validation and Verification

1. Run full CI matrix on all platforms (Linux, macOS, Windows)
2. Verify release workflow (changesets version + publish)
3. Test dev-install workflow
4. Validate devcontainer setup
...
2. Execute Phase 2 (scripts and tooling)
3. Execute Phase 3 (CI workflows)
4. Execute Phase 4 (devcontainer)
