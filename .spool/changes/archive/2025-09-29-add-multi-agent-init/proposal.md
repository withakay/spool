...
- Let `spool init` configure new AI coding tools for projects that already contain an Spool structure.
...
Today `spool init` exits with an error once an `spool/` directory exists. That protects the directory layout, but it blocks
...
1. Detect an existing Spool structure at the start of `spool init` and branch into an "extend" mode instead of exiting.
...
1. Summarize the outcome (created, refreshed, skipped) before exiting with code 0 when work was performed.
   - Include friendly guidance that future updates to shared content still come from `spool update`.

## Out of Scope

- Changing how `spool update` discovers or updates AI tool files.
