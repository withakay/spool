...
- **WHEN** the `spool/` directory already exists
...
- **GIVEN** an `spool/` directory already exists and at least one AI tool file is present
...
`spool init` SHALL treat extend mode with no selected tools as a guarded error.

#### Scenario: Preventing empty extend runs
- **WHEN** Spool is already initialized and the user selects no additional tools
