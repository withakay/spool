...
- \[x\] 1.1 Detect existing Spool structures at the start of `spool init` and enter an extend mode instead of failing.
...
- \[x\] 4.1 Add tests covering rerunning `spool init` to add another tool and the scenario where the user declines to add anything.
