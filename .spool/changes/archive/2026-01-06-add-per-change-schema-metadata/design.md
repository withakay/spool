...
The metadata file (`.spool.yaml`) will be validated with this Zod schema:
...
/**
 * Schema for per-change metadata stored in .spool.yaml
 */
...
- Should `spool new change` prompt for schema interactively if not specified? (Leaning no - default is fine)
