...
**Scope: Experimental artifact workflow only** (`spool new change`, `spool status`, `spool instructions`, `spool templates`)

- Store schema choice in `.spool.yaml` metadata file when creating a change via `spool new change`
...
- Add `--schema` option to `spool new change` command

**Not affected**: Legacy commands (`spool validate`, `spool archive`, `spool list`, `spool show`)
...
- **Backward compatible**: Changes without `.spool.yaml` fall back to `spec-driven` default
