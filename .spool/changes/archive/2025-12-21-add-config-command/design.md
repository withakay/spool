...
The `global-config` spec defines how Spool reads/writes `config.json`, but users currently must edit it by hand. This command provides a CLI interface to that config.
...
spool config set featureFlags.newFlag true
```
