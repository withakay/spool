...
- **Better alignment**: Focuses on the core Spool workflow without redundant features
...
- `/spool/README.md` - Remove diff command references
- Various documentation files mentioning `spool diff`

### Migration Guide for Users

Users currently using `spool diff` should transition to:

```bash
# Before
spool diff add-feature

# After - view the change proposal
spool show add-feature

# After - view only the deltas
spool show add-feature --json --deltas-only

# After - use git for file comparisons
git diff spool/specs spool/changes/add-feature/specs
```
