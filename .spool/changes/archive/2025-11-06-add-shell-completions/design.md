...
spool validate <TAB><TAB>  # Shows: --all --changes --specs --strict --json [change-ids] [spec-ids]
spool show add-<TAB><TAB>  # Shows all changes starting with "add-"
```
...
spool show <TAB>  # Shows menu with all change IDs and spec IDs, categorized
```
...
spool show a    # Real-time suggestions for changes starting with "a"
spool <TAB>     # Shows all commands with descriptions in paged menu
```
...
spool show <TAB>     # Cycles through change IDs one by one
spool <Ctrl+Space>   # Shows IntelliSense menu with all commands
```
...
class CompletionProvider {
  ...
  isSpoolProject(): boolean {
    // Check for spool/ directory
  }
}
```
...
│       ├── command-registry.ts    # Single source of truth for Spool commands
...
if (isOhMyZshInstalled()) {
  ...
  // Install to ~/.zsh/completions/_spool
