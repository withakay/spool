...
- [x] 1.1 Add an "AGENTS.md standard" option to the `spool init` tool-selection prompt, respecting the existing UI conventions.
- [x] 1.2 Generate or refresh a root-level `AGENTS.md` file using the Spool markers when that option is selected, sourcing content from the canonical template.

## 2. Enhance Update Command
- [x] 2.1 Ensure `spool update` writes the root `AGENTS.md` from the latest template (creating it if missing) alongside `spool/AGENTS.md`.
...
- [x] 4.2 Run `spool validate add-agents-md-config --strict` and document any notable behavior changes.
