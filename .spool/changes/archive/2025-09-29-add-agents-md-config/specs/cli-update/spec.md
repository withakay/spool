...
- **THEN** the command SHALL:
  ...
  - Replace `spool/AGENTS.md` with the latest template (complete replacement)
...
- **AND** create or update the root-level `AGENTS.md` using the Spool markers
- **AND** update only the Spool-managed blocks in **existing** AI tool files using markers
- **AND** use the default directory name `spool`
...
- **AND** display the message: "Updated Spool instructions"
