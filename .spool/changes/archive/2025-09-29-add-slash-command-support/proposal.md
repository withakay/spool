...
## Summary
- Enable Spool to generate and update custom slash commands for supported coding agents (Claude Code and Cursor).
- Provide three slash commands aligned with Spool's workflow: proposal (start a change proposal), apply (implement), and archive.
- Share slash command templating between agents to make future extensions simple.

## Motivation
Developers use different coding agents and editors. Having consistent slash commands across tools for the Spool workflow reduces friction and ensures a standard way to trigger the workflow. Supporting both Claude Code and Cursor now lays a foundation for future agents that introduce slash command features.

## Proposal
1. During `spool init`, when a user selects a supported tool, generate slash command configuration for three Spool workflow stages:
   - Claude (namespaced): `/spool/proposal`, `/spool/apply`, `/spool/archive`.
   - Cursor (flat, prefixed): `/spool-proposal`, `/spool-apply`, `/spool-archive`.
   - Semantics:
   ...
   - Each command file MUST embed concise, step-by-step instructions sourced from `spool/README.md` (see Template Content section).
2. Store slash command files per tool:
   - Claude Code: `.claude/commands/spool/{proposal,apply,archive}.md`
   - Cursor: `.cursor/commands/{spool-proposal,spool-apply,spool-archive}.md`
...
3. Command file format and metadata:
   ...
   - Place Spool markers around the body only, never inside frontmatter.
   - Keep the visible slash name, file name, and any frontmatter `name`/`id` consistently aligned (e.g., `proposal`, `spool-proposal`).
   - Namespacing: categorize these under “Spool” and prefer unique IDs (e.g., `spool-proposal`) to avoid collisions.
...
5. During `spool update`, refresh only existing slash command files (per-file basis) within markers; do not create missing files or new tools.
...
- Introduce `SlashCommandConfigurator` to manage multiple files per tool.
  ...
  - Provide `generateAll(projectPath, spoolDir)` for init and `updateExisting(projectPath, spoolDir)` for update.
- Per-tool adapters add only frontmatter and pathing; bodies come from shared templates.
- Templates live in `TemplateManager` with helpers that extract concise, authoritative snippets from `spool/README.md`.
...
- Markers MUST wrap only the Markdown body contents:
  ...
  - Then `<!-- SPOOL:START -->` … body … `<!-- SPOOL:END -->`.
...
- Directory creation for `.claude/commands/spool/` and `.cursor/commands/` is the configurator’s responsibility.

### Command Naming & UX
- Claude Code: use namespacing in the slash itself for readability and grouping: `/spool/proposal`, `/spool/apply`, `/spool/archive`.
- Cursor: use flat names with an `spool-` prefix: `/spool-proposal`, `/spool-apply`, `/spool-archive`. Group via `category: Spool` when supported.
- Consistency: align file names, visible slash names, and any frontmatter `id` (e.g., `id: spool-apply`).
...
- Allow users to customize command names and categories during `spool init`.
...
### Claude Code: `.claude/commands/spool/proposal.md`
```markdown
---
name: Spool: Proposal
...
Slash invocation: `/spool/proposal` (namespaced)

### Cursor: `.cursor/commands/spool-proposal.md`
```markdown
---
name: /spool-proposal
id: spool-proposal
...
Slash invocation: `/spool-proposal` (flat, prefixed)

## Template Content
Templates should be brief, actionable, and sourced from `spool/README.md` to avoid duplication. Each command body includes:
...
- Pointers to `spool show`, `spool list`, and troubleshooting tips when validation fails.
