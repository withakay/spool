...
- \[x\] 1.2 Generate `.agent/workflows/spool-proposal.md`, `spool-apply.md`, and `spool-archive.md` with YAML frontmatter restricted to a single `description` field for each stage and wrap the body in Spool markers.
...
- \[x\] 2.1 Detect existing Antigravity workflow files during `spool update` and refresh only the managed body, skipping creation when files are missing.
