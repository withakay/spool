...
- **WHEN** user runs `spool x-instructions <artifact> --change <id>`
...
- **WHEN** user runs `spool x-instructions <artifact> --change <id> --json`
- **THEN** the system outputs JSON matching ArtifactInstructions interface

#### Scenario: Unknown artifact

- **WHEN** user runs `spool x-instructions unknown-artifact --change <id>`
...
- **WHEN** user runs `spool x-instructions proposal --change <id>` on a scaffolded change
...
The system SHALL generate schema-aware apply instructions via `spool x-instructions apply`.
