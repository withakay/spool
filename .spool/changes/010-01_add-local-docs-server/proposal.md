...
- Add a per-project local web server command group (`spool serve start|stop`) that serves:
  - `.spool/changes/`
  - `.spool/specs/`
  - `.spool/modules/`
...
- Add `spool serve stop` to terminate the server for the current project.
...
- Encourages further CLI modularization to prevent a single `spool-cli` source file from growing without bound.
