...
describe('telemetry/index', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `projector-telemetry-test-${randomUUID()}`);
  ...
  describe('isTelemetryEnabled', () => {
    it('should return false when PROJECTOR_TELEMETRY=0', () => {
      process.env.PROJECTOR_TELEMETRY = '0';
    ...
    it('should return true when no opt-out is set', () => {
      delete process.env.PROJECTOR_TELEMETRY;
      delete process.env.DO_NOT_TRACK;
      delete process.env.CI;
      expect(isTelemetryEnabled()).toBe(true);
    });

    it('should prioritize PROJECTOR_TELEMETRY=0 over other settings', () => {
      process.env.PROJECTOR_TELEMETRY = '0';
      delete process.env.DO_NOT_TRACK;
      delete process.env.CI;
      expect(isTelemetryEnabled()).toBe(false);
