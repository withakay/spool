import { FISH_DYNAMIC_HELPERS, FISH_STATIC_HELPERS } from '../templates/fish-templates.js';
import { CommandDefinition, CompletionGenerator, FlagDefinition } from '../types.js';

/**
 * Generates Fish completion scripts for the Spool CLI.
...
export class FishGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    for (const cmd of commands) {
      ...
      topLevelLines.push(
        `complete -c spool-bun -n '__fish_spool_no_subcommand' -a '${cmd.name}' -d '${this.escapeDescription(cmd.description)}'`
      );
    }
    ...
    return `# Fish completion script for Spool CLI
...
${commandCompletions}`;
  ...
  private generateCommandCompletions(cmd: CommandDefinition): string[] {
    ...
    if (cmd.subcommands && cmd.subcommands.length > 0) {
      ...
      for (const subcmd of cmd.subcommands) {
        lines.push(
          `complete -c spool-bun -n '__fish_spool_using_subcommand ${cmd.name}; and not __fish_spool_using_subcommand ${subcmd.name}' -a '${subcmd.name}' -d '${this.escapeDescription(subcmd.description)}'`
        );
      }
      lines.push('');
  ...
  private generateFlagCompletion(flag: FlagDefinition, condition: string): string[] {
    ...
    if (flag.takesValue && flag.values) {
      ...
      for (const value of flag.values) {
        if (shortFlag) {
          lines.push(
            `complete -c spool-bun -n '${condition}' -s ${flag.short} -l ${flag.name} -a '${value}' -d '${this.escapeDescription(flag.description)}'`
          );
        } else {
          lines.push(
            `complete -c spool-bun -n '${condition}' -l ${flag.name} -a '${value}' -d '${this.escapeDescription(flag.description)}'`
          );
        }
      }
      ...
      if (shortFlag) {
        lines.push(
          `complete -c spool-bun -n '${condition}' -s ${flag.short} -l ${flag.name} -r -d '${this.escapeDescription(flag.description)}'`
        );
      } else {
        lines.push(
          `complete -c spool-bun -n '${condition}' -l ${flag.name} -r -d '${this.escapeDescription(flag.description)}'`
        );
      }
    } else {
      // Boolean flag
      if (shortFlag) {
        lines.push(
          `complete -c spool-bun -n '${condition}' -s ${flag.short} -l ${flag.name} -d '${this.escapeDescription(flag.description)}'`
        );
      } else {
        lines.push(
          `complete -c spool-bun -n '${condition}' -l ${flag.name} -d '${this.escapeDescription(flag.description)}'`
        );
      }
    }
  ...
  private generatePositionalCompletion(
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(`complete -c spool-bun -n '${condition}' -a '(__fish_spool_changes)' -f`);
        break;
      case 'spec-id':
        lines.push(`complete -c spool-bun -n '${condition}' -a '(__fish_spool_specs)' -f`);
        break;
      case 'change-or-spec-id':
        lines.push(`complete -c spool-bun -n '${condition}' -a '(__fish_spool_items)' -f`);
        break;
      case 'shell':
        lines.push(`complete -c spool-bun -n '${condition}' -a 'zsh bash fish powershell' -f`);
        break;
      case 'path':
