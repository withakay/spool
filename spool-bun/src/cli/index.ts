...
function findUpPackageJson(startDir: string): string {
  let dir = startDir;
  for (let i = 0; i < 20; i++) {
...
const pkgJsonPath = findUpPackageJson(currentDir);
...
function getCommandPath(command: Command): string {
  ...
  while (current) {
    ...
    // Skip the root 'spool-bun' command
    if (name && name !== 'spool-bun') {
      names.unshift(name);
    }
    current = current.parent;
  }

  return names.join(':') || 'spool-bun';
}

program
  .name('spool-bun')
  .description('Deprecated legacy TypeScript/Bun implementation of Spool')
  .version(version);
