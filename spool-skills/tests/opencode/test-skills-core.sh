...
# Create skills in personal and spoolskills locations for testing
mkdir -p "$TEST_HOME/personal-skills/shared-skill"
mkdir -p "$TEST_HOME/spoolskills/shared-skill"
mkdir -p "$TEST_HOME/spoolskills/unique-skill"
...
cat > "$TEST_HOME/spoolskills/shared-skill/SKILL.md" <<'EOF'
---
name: shared-skill
description: Spoolskills version
---
# Spoolskills Shared
EOF

cat > "$TEST_HOME/spoolskills/unique-skill/SKILL.md" <<'EOF'
---
name: unique-skill
description: Only in spoolskills
---
# Unique
EOF
...
function resolveSkillPath(skillName, spoolskillsDir, personalDir) {
    const forceSpoolskills = skillName.startsWith('spoolskills:');
    const actualSkillName = forceSpoolskills ? skillName.replace(/^spoolskills:/, '') : skillName;

    if (!forceSpoolskills && personalDir) {
    ...
    if (spoolskillsDir) {
        const spoolskillsPath = path.join(spoolskillsDir, actualSkillName);
        const spoolskillsSkillFile = path.join(spoolskillsPath, 'SKILL.md');
        if (fs.existsSync(spoolskillsSkillFile)) {
            return {
                skillFile: spoolskillsSkillFile,
                sourceType: 'spoolskills',
                skillPath: actualSkillName
            };
        }
    }
...
const spoolskillsDir = '$TEST_HOME/spoolskills';
const personalDir = '$TEST_HOME/personal-skills';

// Test 1: Shared skill should resolve to personal
const shared = resolveSkillPath('shared-skill', spoolskillsDir, personalDir);
console.log('SHARED:', JSON.stringify(shared));

// Test 2: spoolskills: prefix should force spoolskills
const forced = resolveSkillPath('spoolskills:shared-skill', spoolskillsDir, personalDir);
console.log('FORCED:', JSON.stringify(forced));

// Test 3: Unique skill should resolve to spoolskills
const unique = resolveSkillPath('unique-skill', spoolskillsDir, personalDir);
...
if echo "$result" | grep -q 'SHARED:.*"sourceType":"personal"'; then
    echo "  [PASS] Personal skills shadow spoolskills"
else
...
if echo "$result" | grep -q 'FORCED:.*"sourceType":"spoolskills"'; then
    echo "  [PASS] spoolskills: prefix forces spoolskills resolution"
else
    echo "  [FAIL] spoolskills: prefix not working"
    exit 1
fi

if echo "$result" | grep -q 'UNIQUE:.*"sourceType":"spoolskills"'; then
    echo "  [PASS] Unique spoolskills are found"
else
    echo "  [FAIL] Unique spoolskills not found"
    exit 1
fi
