...
# Verifies that skills are resolved with correct priority: project > personal > spoolskills
...
# 1. Create in spoolskills location (lowest priority)
mkdir -p "$HOME/.config/opencode/spool-skills/skills/priority-test"
cat > "$HOME/.config/opencode/spool-skills/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Spoolskills version of priority test skill
---
# Priority Test Skill (Spoolskills Version)

This is the SPOOLSKILLS version of the priority test skill.

PRIORITY_MARKER_SPOOLSKILLS_VERSION
EOF
...
if [ -f "$HOME/.config/opencode/spool-skills/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Spoolskills version exists"
else
    echo "  [FAIL] Spoolskills version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides spoolskills
echo ""
echo "Test 2: Testing personal > spoolskills priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides spoolskills)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_SPOOLSKILLS_VERSION"; then
    echo "  [FAIL] Spoolskills version loaded instead of personal"
    exit 1
else
...
# Test 3: Test that project overrides both personal and spoolskills
echo ""
echo "Test 3: Testing project > personal > spoolskills priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Spoolskills version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit spoolskills: prefix bypasses priority
echo ""
echo "Test 4: Testing spoolskills: prefix forces spoolskills version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load spoolskills:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_SPOOLSKILLS_VERSION"; then
    echo "  [PASS] spoolskills: prefix correctly forces spoolskills version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] spoolskills: prefix did not force spoolskills version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
