...
# Use projector __complete to get available changes
_projector_complete_changes() {
  local -a changes
  while IFS=$'\\t' read -r id desc; do
    changes+=("$id:$desc")
  done < <(projector __complete changes 2>/dev/null)
  _describe "change" changes
}

# Use projector __complete to get available specs
_projector_complete_specs() {
  local -a specs
  while IFS=$'\\t' read -r id desc; do
    specs+=("$id:$desc")
  done < <(projector __complete specs 2>/dev/null)
