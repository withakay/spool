...
# Use spool __complete to get available changes
_spool_complete_changes() {
  local -a changes
  while IFS=$'\\t' read -r id desc; do
    changes+=("$id:$desc")
  done < <(spool __complete changes 2>/dev/null)
  _describe "change" changes
}

# Use spool __complete to get available specs
_spool_complete_specs() {
  local -a specs
  while IFS=$'\\t' read -r id desc; do
    specs+=("$id:$desc")
  done < <(spool __complete specs 2>/dev/null)
