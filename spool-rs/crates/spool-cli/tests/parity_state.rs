...
#[test]
fn parity_state_decision_matches_oracle() {
    let base = make_base_repo();
    let repo = tempfile::tempdir().expect("work");
    let home = tempfile::tempdir().expect("home");
    let rust_path = assert_cmd::cargo::cargo_bin!("spool");

    let plan_init = ["plan", "init"];
    let decision = ["state", "decision", "Decide", "thing"];

    reset_repo(repo.path(), base.path());
    run_ts_oracle(&plan_init, repo.path(), home.path());
    let ts = run_ts_oracle(&decision, repo.path(), home.path()).normalized(home.path());
    let ts_fs = collect_file_bytes(repo.path());

    reset_repo(repo.path(), base.path());
    run_rust_candidate(rust_path, &plan_init, repo.path(), home.path());
    let rs =
        run_rust_candidate(rust_path, &decision, repo.path(), home.path()).normalized(home.path());
