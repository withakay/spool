...
#[test]
fn deprecated_instructions_apply_prints_apply_text() {
    let base = fixtures::make_repo_with_spec_change_fixture();
    let repo = tempfile::tempdir().expect("work");
    let home = tempfile::tempdir().expect("home");
    let rust_path = assert_cmd::cargo::cargo_bin!("spool");
    ...
    let out = run_rust_candidate(
        rust_path,
        &["instructions", "apply", "--change", "000-01_test-change"],
        repo.path(),
        home.path(),
    );
    assert_eq!(out.code, 0);
    assert!(out.stdout.contains("## Apply:"));
    assert!(out.stdout.contains("### Instruction"));
}

#[test]
fn deprecated_instructions_missing_artifact_and_bad_artifact_show_hints() {
    let base = fixtures::make_repo_with_spec_change_fixture();
    let repo = tempfile::tempdir().expect("work");
    let home = tempfile::tempdir().expect("home");
    let rust_path = assert_cmd::cargo::cargo_bin!("spool");
    ...
    let out = run_rust_candidate(
        rust_path,
        &["instructions", "--change", "000-01_test-change"],
        repo.path(),
        home.path(),
    );
    ...
    assert_ne!(out.code, 0);
    assert!(out.stderr.contains("Valid artifacts"));
}
